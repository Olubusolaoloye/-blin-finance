...
import { Copy, ChevronRight, ShieldCheck, HelpCircle, LogOut, Settings, PiggyBank, Lock, Globe, FileText, Bell, Camera, User, Mail, CheckCircle2 } from 'lucide-react';
      ...
      <BlinCard variant="dark" className="bg-gradient-to-br from-[#0D2137] to-[#1A3C6E] p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-display font-semibold text-[22px] text-white leading-tight">{user.firstName} {user.lastName}</h2>
              <p className="text-[13px] text-white/60 mb-2">{user.email}</p>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-white rounded-full text-[11px] font-semibold text-brand-navy">
        ...
        <div className="flex items-center justify-between bg-white/10 rounded-xl p-3 backdrop-blur-sm mb-4">
          <div className="flex items-center gap-2">
            <span className="font-mono text-[13px] text-white/80">{user.walletAddress}</span>
        ...
        <div className="flex items-center justify-between">
          <span className="text-[12px] text-white/50">Joined {user.joined}</span>
          <div className="flex items-center gap-2">
            <span className="text-[12px] text-brand-gold font-bold">FlowScore: {user.flowScore}</span>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="bg-white rounded-[16px] border border-border-light overflow-hidden">
            <button onClick={() => navigate('/vault')} className="w-full flex items-center justify-between p-4 border-b border-border-light hover:bg-surface-raised transition-colors">
            ...
            <button onClick={() => navigate('/vault')} className="w-full flex items-center justify-between p-4 hover:bg-surface-raised transition-colors">
              <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="bg-white rounded-[16px] border border-border-light overflow-hidden">
            {[
              { key: 'transactions', label: 'Successful Transactions', desc: 'Alert when money enters or leaves wallet' },
              { key: 'market', label: 'Market Alerts', desc: 'Price fluctuations and trending stocks' },
              { key: 'yield', label: 'Vault & Yield', desc: 'Notifications on vault lock and weekly yield' },
            ].map((item, i) => (
        ...
        <div>
          ...
          <div className="bg-white rounded-[16px] border border-border-light overflow-hidden">
            <button onClick={() => navigate('/tax')} className="w-full flex items-center justify-between p-4 border-b border-border-light hover:bg-surface-raised transition-colors">
              <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col pt-4">
          ...
          <div className="space-y-6 mb-10">
            <div className="flex flex-col gap-2">
              <label className="text-[12px] font-bold text-text-secondary uppercase tracking-wider ml-1">First Name</label>
              <div className="relative">
                <input 
                  type="text"
                  value={editFormData.firstName}
                  onChange={(e) => setEditFormData(prev => ({ ...prev, firstName: e.target.value }))}
                  className="w-full h-14 bg-surface-raised rounded-2xl px-5 font-semibold text-[16px] outline-none border-2 border-transparent focus:border-brand-blue transition-all"
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[12px] font-bold text-text-secondary uppercase tracking-wider ml-1">Last Name</label>
              <div className="relative">
                <input 
                  type="text"
          ...
          <div className="flex flex-col gap-3">
            <BlinButton 
              className="w-full h-14 rounded-2xl font-bold text-[16px]" 
