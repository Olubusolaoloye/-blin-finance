...
export default function TaxCalculator() {
  ...
  const [taxRate, setTaxRate] = useState(10); // 10% default crypto tax rate

  // Calculate tax based on mock transactions (sells/swaps)
  const taxData = useMemo(() => {
    const taxableEvents = mockTransactions.filter(tx => tx.type === 'sell' || tx.type === 'swap');
    ...
    taxableEvents.forEach(tx => {
      // Use mock data if available, else simulate
      const proceeds = parseFloat(tx.usdValue.replace(/[^0-9.-]+/g,""));
      const cost = tx.costBasis || (proceeds * 0.7); // Simulate 30% profit if no cost basis
      const gain = tx.capitalGain || (proceeds - cost);
      ...
      <div>
        ...
        <div className="bg-white rounded-[16px] border border-border-light overflow-hidden">
          {taxData.events.map((tx, i) => (
            <div key={i} className="flex items-center justify-between p-4 border-b border-border-light last:border-0 hover:bg-surface-raised transition-colors">
              ...
              <div className="text-right">
                <div className="font-bold text-[14px] text-brand-green">
                  +<AmountDisplay amount={tx.capitalGain || (parseFloat(tx.usdValue.replace(/[^0-9.-]+/g,"")) * 0.3)} currency="USD" />
                </div>
                <div className="text-[12px] text-text-muted">Gain</div>
              </div>
            </div>
          ))}
          {taxData.events.length === 0 && (
