...
export function SideNavBar({ isOpen, onClose, isCollapsed, onToggleCollapse }: SideNavBarProps) {
  const pathname = usePathname();
  const { shortAddress, initials, displayName, email, disconnect } = useAuth();
    ...
    >
      ...
      <div className={cn("p-8 flex items-center justify-between", isCollapsed && "px-4 justify-center")}>
        {!isCollapsed ? (
          ...
          <div className="flex items-center justify-center bg-white/10 w-10 h-10 rounded-xl font-display font-bold text-brand-gold">
            B
          </div>
        )}
        <button
      ...
      <div className={cn("p-6 border-t border-white/10", isCollapsed && "p-4")}>
        <div className={cn("flex items-center gap-3 mb-4", isCollapsed && "justify-center")}>
          <div className="w-10 h-10 rounded-full bg-brand-blue flex items-center justify-center font-bold text-[13px] shrink-0">
            {initials}
          </div>
          {!isCollapsed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col overflow-hidden">
              <span className="text-[14px] font-bold truncate">{displayName || shortAddress || "Not connected"}</span>
              <span className="text-[12px] text-white/50 truncate">{email || "Web3 Wallet"}</span>
            </motion.div>
          )}
        </div>
        <button
